    ...
    async function loadSettings() {
      ...
      document.querySelector("header #webtitle").textContent = set.apititle
       document.getElementById("webtitle").textContent = set.apititle
        ...
        let menuHTML = `<a class="block py-2.5 px-4 rounded hover:bg-[#1f1f2e] text-gray-100 font-poppins font-normal text-[18px]" href=${window.location.origin}/><i class="fas fa-tachometer-alt"></i> Dashboard</a>`;

        settings.forEach((item, index) => {
            menuHTML += `
                <div class="relative font-poppins font-normal">
                    <button class="block py-2.5 px-4 rounded text-gray-100 hover:bg-[#1f1f2e] w-full text-left text-[17px]" onclick="toggleDropdown('dropdown-${index}', 'chevron-${index}')">
                    ...
                    <div id="dropdown-${index}" class="hidden">
                        ${item.api.slice().sort((a, b) => a.name.localeCompare(b.name)).map(api => `<a class="flex items-center gap-1 text-gray-100 py-2.5 px-4 font-[17px] hover:bg-[#1f1f2e]" href=${window.location.origin}${api.path}><i class="fa-regular fa-circle mr-1 ml-1 font-bold text-[8px]"></i>${api.name}</a>`).join('')}
                    </div>
                </div>`;
        });
    ...
    document.addEventListener('click', (event) => {
        ...
        navigator.getBattery().then(battery => {
            ...
            document.getElementById('tqto').innerHTML = set.developer.map(e => `<div class="text-gray-100 font-normal"><a class="flex items-center" href=${e.contact}>— ${e.name}</a></div>`).join("\n")
